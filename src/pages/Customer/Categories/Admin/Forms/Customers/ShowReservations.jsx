...
export default function ShowReservation({ handleClose }) {
    ...
    useEffect(() => {
        ...
        cipherRequest(toSend, `${config.api}/customer/getAllUsers`).then((res) => {
            setUsers(res.data)
        })
    }, [])
	...
	const handleSelect = (user) => {
        if(selectedUser._id !== user._id) {
            const toSend = JSON.stringify({token: localStorage.getItem("katiacm"), userId: user._id})
    ...
    const handleDesactivateReservation = (id) => {
        ...
        cipherRequest(toSend, `${config.api}/reservation/desactivateReservations`).then((res) => {
            if(res.status === 0) {
                for(let i = 0; i <= selectedUser.reservations.length-1; ++i) {
